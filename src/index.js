...
async function main() {
    const debridStatus = config.debridService === 'none'
        ? 'DISABLED'
        : (config.debridService === 'torbox' ? config.isTorboxEnabled : config.isRdEnabled)
            ? 'ENABLED'
            : 'DISABLED (no key)';
    logger.info(`Debrid provider: ${config.debridService} — ${debridStatus}`);
